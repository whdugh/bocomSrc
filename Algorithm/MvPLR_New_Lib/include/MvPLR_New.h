...
#ifdef DEBUG
#define WHITE_DEBUG
//#define WHITE_SAVE_SAMPLE //保存白牌字符
//#define TIME_START
//#define SAVE_RESULT 
//#define SAVE_FAKE_PLATE
//#define SAVE_SAMPLE //保存样本for陈博
...
//#define WHITE_DEBUG_DEBUG //白牌打印和显示
//#define WHITE_DEBUG_DEBUG_SAVEIMAGE //白牌存储样本图片
#endif
...
static char whiteLabels[] = {
	'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j',
	'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't','u', 'v',
	'w', 'x', 'y', 'z', 'A', 'B', 'C', 'D', 'E',
	'Q','S','R','X','U','Y','O','V','T','W','P','L','Z'
};
...
public:
	...
	int mv_PLRmain( IplImage* InImg, Mv_Vehicle *VehicleDet,int VehicleNum,vector<carnum_context> &PLR_result,char *SaveSamplePath);
private:
	//int mv_PLR_Recognition( IplImage*, CvRect*, int, MvPlateinfo& );
	int mv_PLR_Recognition( IplImage*, CvRect*, int, carnum_context& );
	int mv_PLR_Recognition_White( IplImage* , CvRect* , char*, double* );
	int mv_PLR_Recognition_White( IplImage*, CvRect, char& , double& );
	...
	// 白牌DL分类器对象
	ConvNet* mv_DbnRec_white;
	CvMat* convetdst_white;
