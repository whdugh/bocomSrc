...
#else
		return (setsockopt(nSocket, SOL_SOCKET, SO_SNDTIMEO, (char*)&timeout, sizeof(struct timeval)) == 0);
#endif
    }
...
#else
		return (setsockopt(nSocket, SOL_SOCKET, SO_SNDBUF, (char*)&nLen, sizeof(int)) == 0);
#endif
    }
...
#else
		 return (setsockopt(nSocket, SOL_SOCKET, SO_RCVBUF, (char*)&nLen, sizeof(int)) == 0);
#endif
    }
    else
    {
...
#else
		if(setsockopt(nSocket, SOL_SOCKET, SO_SNDTIMEO, (char*)&timeo, len) == -1)
#endif
        {
            perror("setsockopt");
            return false;
...
bool mvCSocketBase::mvSendMsgToSocket(int nSocket, const string& strMsg,bool bBlock)
{
...
#else
		if ((nBytes = send(nSocket,(char*) (strMsg.c_str()+nDataLen), nLeft>16384?16384:nLeft, 0)) < 0)
#endif
        {
            //解锁缓冲区
    #ifdef WIN32
	...
	pthread_mutex_unlock(&m_SendMutex);
#endif
            printf2File("send error===nSocket=%d==nLeft=%d,nDataLen=%d,%s\n",nSocket, nLeft,nDataLen,strerror(errno));
            return false;
        }
        nDataLen += nBytes;
        nLeft -= nBytes;
