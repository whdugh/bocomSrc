...
////////////////////////////////////////////////////////////////////////////////
/**
 * Rotates Left 90 image around it's center.
 * \param  ippMethod - IPP interpolation method used
 ...
 *
 */
bool CxImage::IppRotateLeft(IPPInterMethod const ippMethod, CxImage* iDst)
{
	if (!pDib) return false;
	...
	imgDest.Create(newWidth,newHeight,GetBpp(),GetType());
	imgDest.SetPalette(GetPalette());
	if (!imgDest.IsValid()) return false;
	...
	imgSize    = roi.width * roi.height * channels;
	
	int dstImgStep = newWidth*channels + DIB_PAD_BYTES(newWidth,channels);
	int dstImgSize = dstImgStep*newHeight;
	
	IppiRect srcroi={0,0,roi.width,roi.height};
	IppiRect dstroi={0,0,newWidth,newHeight};
	
	double angle = -90;
	double center = (double)roi.height/2;

	if(channels == 3)
		ippiRotateCenter_8u_C3R(this->GetBits(), roi, lineStep, srcroi, imgDest.info.pImage, dstImgStep, dstroi,angle,center, center, ippMethod );
	else if(channels == 1)
		ippiRotateCenter_8u_C1R(this->GetBits(), roi, lineStep, srcroi, imgDest.info.pImage, dstImgStep, dstroi,angle,center, center, ippMethod );
...
////////////////////////////////////////////////////////////////////////////////
/**
 * Rotates 180 image around it's center.
 * \param  ippMethod - IPP interpolation method used
 * \param  *iDst - destination image (if null, this image is changed)
 * \param  vertical
 ...
 *
 */
bool CxImage::IppRotate180(CxImage* iDst, bool vertical)
{
	if (!pDib) return false;
	...
	{
	case 1:
		ippiMirror_8u_C1IR(info.pImage,lineStep,roi,flip);
		break;
		
	case 3:
		ippiMirror_8u_C3IR(info.pImage,lineStep,roi,flip);
		break;
		
	case 4:
		ippiMirror_8u_C4IR(info.pImage,lineStep,roi,flip);
		break;
...
////////////////////////////////////////////////////////////////////////////////
/**
 * Rotates Right 90 image around it's center.
 * \param  ippMethod - IPP interpolation method used
 * \param  *iDst - destination image (if null, this image is changed)
 ...
 *
 */
bool CxImage::IppRotateRight(IPPInterMethod const ippMethod, CxImage* iDst)
{
	if (!pDib) return false;
	...
	double angle = 90;
	double center = (double)roi.width/2;
 ...
 *
 */
bool CxImage::IppRotate(float angle, IPPInterMethod const ippMethod, CxImage* iDst)
{
	if (!pDib) return false;
	...
	CxImage imgDest;
	imgDest.CopyInfo(*this);
	imgDest.Create(head.biWidth,head.biHeight,GetBpp(),GetType());
	imgDest.SetPalette(GetPalette());
	if (!imgDest.IsValid()) return false;
	...
	double centerX = (double)roi.width/2;
	double centerY = (double)roi.height/2;
	double xShift,yShift;

	ippiGetRotateShift(centerX,centerY,angle,&xShift,&yShift);

	if(channels == 3)
		ippiRotate_8u_C3R(this->GetBits(), roi, lineStep, srcroi, imgDest.info.pImage, lineStep, srcroi,angle,xShift, yShift, ippMethod );
	else if(channels == 1)
		ippiRotate_8u_C1R(this->GetBits(), roi, lineStep, srcroi, imgDest.info.pImage, lineStep, srcroi,angle,xShift, yShift, ippMethod );
 ...
 * \param  ippMethod The type of interpolation to perform for resampling the image.
 ...
 *
 */
bool CxImage::IppResize(long newx, long newy, IPPInterMethod const ippMethod, CxImage* iDst)
{
	//按比例缩放
	...
	CxImage imgDest;
	imgDest.CopyInfo(*this);
	imgDest.Create(newx,newy,head.biBitCount,GetType());
	imgDest.SetPalette(GetPalette());
	if (!imgDest.IsValid()) return false;
	...
	double   m_ratioX = (double)newx / (double)roi.width ;
	double   m_ratioY = (double)newy / (double)roi.height ;
	int dstImgStep = newx*channels + DIB_PAD_BYTES(newx,channels);
	int dstImgSize = dstImgStep*newy;
	...
	/*if(channels == 3)
		ippiResize_8u_C3R(this->GetBits(), roi, lineStep, srcroi, imgDest.info.pImage, dstImgStep, dstRoi,m_ratioX, m_ratioY, ippMethod );
	else if(channels == 1)
		ippiResize_8u_C1R(this->GetBits(), roi, lineStep, srcroi, imgDest.info.pImage, dstImgStep, dstRoi,m_ratioX, m_ratioY, ippMethod );
	*/
	//select the destination
	if (iDst) iDst->Transfer(imgDest);
	else Transfer(imgDest);
 ...
 * \param  &rect - long left, long top, long right, long bottom .
 * \param  ippMethod The type of interpolation to perform for resampling the image.
 * \param  *iDst - destination image (if null, this image is changed)
 *
 * Shear ippMethod:
 ...
 *
 */
bool CxImage::IppShear(const RECT& rect, IPPInterMethod const ippMethod, CxImage* iDst)
{
	return IppShear(rect.left, rect.top, rect.right, rect.bottom, ippMethod, iDst);
}
////////////////////////////////////////////////////////////////////////////////
bool CxImage::IppShear(long left, long top, long right, long bottom, IPPInterMethod const ippMethod, CxImage* iDst)
{
	if (!pDib) return false;
	...
	CxImage imgDest;
	imgDest.CopyInfo(*this);
	imgDest.Create(newWidth,newHeight,head.biBitCount,GetType());
	imgDest.SetPalette(GetPalette());
	if (!imgDest.IsValid()) return false;
	...
	IppiRect srcroi={startx,starty,endx,endy};
	IppiRect dstroi={0,0,newWidth,newHeight};
	...
	double bound[2][2];
	bound[0][0] = (double)left;
	bound[0][1] = (double)top;
	bound[2][0] = (double)right;
	bound[2][1] = (double)bottom;

	ippiGetShearBound(srcroi,bound,xShear,yShear,xShift,yShift);

	if(channels == 3)
		ippiShear_8u_C3R(this->GetBits(), roi, lineStep, srcroi, imgDest.info.pImage, dstImgStep, dstroi, xShear, yShear, xShift, yShift, ippMethod );
	else if(channels == 1)
		ippiShear_8u_C1R(this->GetBits(), roi, lineStep, srcroi, imgDest.info.pImage, dstImgStep, dstroi, xShear, yShear, xShift, yShift, ippMethod );
...
/**
 * 默认水印.
 * \param  imgWatMarking - CxImage 水印图.
 */
bool CxImage::WaterMarking(CxImage* imgWatMarking)
{
	//右下角加上seekpai水印
	
	if (!imgWatMarking)return false;
	
	CxImage tmpImgFrm;
	tmpImgFrm.Copy(*imgWatMarking);
	
	try
	{
		//24位
		if (!tmpImgFrm.IsGrayScale()) tmpImgFrm.IncreaseBpp(24);
		if (!this->IsGrayScale()) this->IncreaseBpp(24);
		
		int width    = head.biWidth;
		//int height   = head.biHeight;
		int frmWidth    = tmpImgFrm.head.biWidth;
		//int frmHeight   = tmpImgFrm.head.biHeight;

		//把255透明为100
		/*if (!tmpImgFrm.AlphaIsValid()){
			tmpImgFrm.AlphaCreate();
		}
		tmpImgFrm.AlphaSetMax(100);*/
		...
		if(width >= height)
		{
			m_ratio = (double)width/(double)512;
			dstWidth = frmWidth * m_ratio;
			dstHeight = frmHeight * m_ratio;			
		}
		else
		{
			m_ratio = (double)height/(double)512;
			dstWidth = frmWidth * m_ratio;
			dstHeight = frmHeight * m_ratio;	
		}
		tmpImgFrm.Resample2(dstWidth,dstHeight);*/
		
		int offsetX = -(width - frmWidth);	//down right
		
		bool m_mixalpha = 0;
		Mix(tmpImgFrm,(CxImage::ImageOpType)OpScreen,offsetX,0,m_mixalpha!=0);
 ...
 * \param  imgWatMarking - CxImage 水印图.
 */
bool CxImage::WaterMarking(CxImage* imgWatMarking, long lXOffset, long lYOffset, int bTransparent, RGBQUAD transRGB)
{
	//右下角加上seekpai水印
	...
	try
	{
		...
		//透明(255,255,255,0)
		//tmpImgFrm.SetTransIndex(0);
		//RGBQUAD c={255,255,255,0};
		//tmpImgFrm.SetTransColor(c);

		// 0 <= bTransparent <= 100
		if(bTransparent<0)bTransparent = 0;
		if(bTransparent>100)bTransparent = 100;
		
		bool m_mixalpha = 0;
		Mix(tmpImgFrm,(CxImage::ImageOpType)OpTransparent,-lXOffset,-lYOffset,m_mixalpha!=0,bTransparent,&transRGB);
 ...
 * \param  
 */
//bool CxImage::IppCopy(CxImage* iSrc,long lOffset,  CxImage* iDst, long lYOffset)
//{
	//内置相框
	
	/*int lineStep   = head.biWidth * 3 + DIB_PAD_BYTES(head.biWidth, 3);
	
	IppiSize   dstRoi;
	dstRoi.width = head.biWidth;
	dstRoi.height = head.biHeight;
	
	ippiCopy_8u_C3R( imgFrame->info.pImage, lineStep, info.pImage , lineStep, dstRoi);*/
...
/**
 * 内置相框.
 * \param  imgFrame - CxImage 相框图.
 */
bool CxImage::FrameIn(CxImage* imgFrame)
{
	//内置相框
	...
	CxImage tmpImgFrm;
	tmpImgFrm.Copy(*imgFrame);

	try
	{
		//24位
		if (!tmpImgFrm.IsGrayScale()) tmpImgFrm.IncreaseBpp(24);
		//tmpImgFrm.SetTransIndex(-1);
		
		//long x = tmpImgFrm.GetWidth()/2;
		//long y = tmpImgFrm.GetHeight()/2;
		
		/*if (tmpImgFrm.IsInside(x,y))
		{
			long yflip = tmpImgFrm.GetHeight() - y - 1;
			//tmpImgFrm.SetTransIndex(tmpImgFrm.GetPixelIndex(x,yflip));
			// <DP> RGB transparency
			//tmpImgFrm.SetTransColor(tmpImgFrm.GetPixelColor(x,yflip));*/
		    tmpImgFrm.SetTransIndex(0);
			RGBQUAD c={255,255,255,0};
			tmpImgFrm.SetTransColor(c);
			// resample
			tmpImgFrm.IppResize((long)head.biWidth,(long)head.biHeight);	
		//}
		
		bool m_mixalpha = 1;
		
		Mix(tmpImgFrm,(CxImage::ImageOpType)OpScreen,0,0,m_mixalpha!=0);
 ...
 * \param  imgFrame - CxImage 相框图.
 */
bool CxImage::FrameOut(long lTopX, long lTopY, long rBottomX, long rBottomY, CxImage* imgFrame)
{
	//外置相框
	
	if (!imgFrame)return false;

	if(head.biBitCount < 24)return false;//24位以下的图片暂时不加框

	try
	{
		long   imgSrcWidth  = head.biWidth;
		long   imgSrcHeight = head.biHeight;
		int    channels     = head.biBitCount >> 3;
		long   wTimes;
		long   hTimes;
		bool   bAdtSize;   //最后形成的相框是否要调整大小
		double m_ratio = 1;    //根据图片大小调整相框大小的比例
		...
		{
			if(imgSrcWidth>500)
				m_ratio = (double)(imgSrcWidth * 0.1) / (double)lTopX;
			else
				m_ratio = (double)(imgSrcWidth) * 0.2 / (double)lTopX;
		}
		else
		{
			if(imgSrcHeight>500)
				m_ratio = (double)(imgSrcHeight * 0.1) / (double)lTopY;
			else
				m_ratio = (double)(imgSrcHeight) * 0.2 / (double)lTopY;
		}
		...
		frameHeight = imgFrame->GetHeight() * m_ratio;
		frameWidth = imgFrame->GetWidth() * m_ratio;
		...
		try
		{
			tmpImgFrm.IppResize(frameWidth,frameHeight);
		
		long wCenter = rBottomX - lTopX;
		long hCenter = rBottomY - lTopY;

		if(imgSrcWidth%wCenter > 0)
		{
			wTimes = imgSrcWidth/wCenter + 1;
			bAdtSize = true;
		}
		else
		{
			wTimes = imgSrcWidth/wCenter;
			bAdtSize = false;
		}

		if(imgSrcHeight%hCenter > 0)
			hTimes = imgSrcHeight/hCenter + 1;
		else
			hTimes = imgSrcHeight/hCenter;
		
		long dstWidth  = lTopX*2+wCenter*wTimes;
		long dstHeight = lTopY*2+hCenter*hTimes;

		CxImage imgDest;
		imgDest.CopyInfo(*this);
		imgDest.Create(dstWidth,dstHeight,head.biBitCount,GetType());
		imgDest.SetPalette(GetPalette());
		if (!imgDest.IsValid()) return false;

		/* ------ 开始裁减粘贴到新图片 ------ */

		//long frameWidth  = tmpImgFrm.GetWidth();
		//long frameHeight = tmpImgFrm.GetHeight();
		
		int lineStep   = frameWidth * channels + DIB_PAD_BYTES(frameWidth, channels);
		...
		long srcExc = rBottomY*lineStep ;
		long dstExc = (dstHeight-lTopY)*dstImgStep;

		ippiCopy_8u_C3R( tmpImgFrm.info.pImage + srcExc, lineStep, imgDest.info.pImage + dstExc, dstImgStep, dstRoi);
		
		
		//裁贴相框右上角

		srcExc = rBottomY*lineStep + rBottomX*channels ;
		dstExc = (dstHeight-lTopY)*dstImgStep + (dstWidth - lTopX)*channels ;
		
		ippiCopy_8u_C3R( tmpImgFrm.info.pImage + srcExc, lineStep, imgDest.info.pImage + dstExc, dstImgStep, dstRoi);
		
		//裁贴相框左下角
		
		ippiCopy_8u_C3R( tmpImgFrm.info.pImage , lineStep, imgDest.info.pImage , dstImgStep, dstRoi);
		
		//裁贴相框右下角

		srcExc = rBottomX*channels ;
		dstExc = (dstWidth - lTopX)*channels ;
		...
		dstRoi.width = wCenter;
		dstRoi.height = lTopY;

		for(i=0;i<wTimes;i++)
		{
			//裁贴相框上横框
			ippiCopy_8u_C3R( tmpImgFrm.info.pImage + rBottomY*lineStep + lTopX*channels, lineStep, imgDest.info.pImage + (dstHeight-lTopY)*dstImgStep + (lTopX+i*wCenter)*channels, dstImgStep, dstRoi);

			//裁贴相框下横框
			ippiCopy_8u_C3R( tmpImgFrm.info.pImage + lTopX*channels, lineStep, imgDest.info.pImage + (lTopX+i*wCenter)*channels, dstImgStep, dstRoi);
		}
		...
		{
			//裁贴相框左竖框
			ippiCopy_8u_C3R( tmpImgFrm.info.pImage + lTopY*lineStep , lineStep, imgDest.info.pImage + (lTopY+i*hCenter)*dstImgStep, dstImgStep, dstRoi);
			
			//裁贴相框右竖框
			ippiCopy_8u_C3R( tmpImgFrm.info.pImage + lTopY*lineStep + rBottomX*channels, lineStep, imgDest.info.pImage + (lTopY+i*hCenter)*dstImgStep + (dstWidth - lTopX)*channels, dstImgStep, dstRoi);
		...
		if(bAdtSize)
		{
			double ratioX =(double) imgSrcWidth  / (double)(wCenter * wTimes) ;
			double ratioY =(double) imgSrcHeight / (double)(hCenter * hTimes) ;

			lTopX *= ratioX;
			lTopY *= ratioY;

			imgDest.IppResize((long)imgSrcWidth+lTopX*2,(long)imgSrcHeight+lTopY*2);

			dstImgStep = imgDest.GetWidth() * channels + DIB_PAD_BYTES(imgDest.GetWidth(), channels);
		}

		dstRoi.width = imgSrcWidth;
		dstRoi.height = imgSrcHeight;

		lineStep   = imgSrcWidth * channels + DIB_PAD_BYTES(imgSrcWidth, channels);
		
		ippiCopy_8u_C3R( this->info.pImage , lineStep, imgDest.info.pImage + lTopY*dstImgStep + lTopX*channels, dstImgStep, dstRoi);
