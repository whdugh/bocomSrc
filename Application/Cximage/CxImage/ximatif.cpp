...
//	if(bitspersample > 8) bitspersample = 8;
	...
	if (isRGB){
		...
		if ((photometric==PHOTOMETRIC_MINISBLACK)||(photometric==PHOTOMETRIC_MINISWHITE)||(
			photometric==PHOTOMETRIC_PALETTE)){              //更改
			if	(bitspersample == 1){
	...
	if (isRGB) {
		...
		RGBQUAD *pal;
		//更改16位调色板 --------------------徐永丰 2007-3-17
		pal=(RGBQUAD*)calloc(1<<bitspersample,sizeof(RGBQUAD));
		...
		//long bitsize = head.biSizeImage*samplesperpixel; //更改
		long bitsize = head.biSizeImage*samplesperpixel;//再更改
		if (bitspersample==16) bitsize = bitsize*2;//再更改
