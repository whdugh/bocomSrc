...
//	bool Save(const char * imageFileName){ return CxImage::Save(imageFileName,CXIMAGE_FORMAT_JPG);}
	unsigned char* IppDecode(const char* filename,int * ippLen,int& sz);
	bool  Decode(CxFile * hFile);
	bool  IppThumbDecode(CxFile * hFile);
	bool  IppDecode(CxFile * hFile);
	bool  CxDecode(CxFile * hFile);
	//bool GetExif(CxFile * hFile);
	unsigned char* ReadIppJPEG(CxFile* hFile, int& imageLength, int iMax, int& sz);
	unsigned char* ReadJpgToBmp(unsigned char* pJPEG,int * ippLen,int& sz);
	unsigned char* IppExifThumb(CxFile* hFile, int * ippLen);
	unsigned char* IppJpgThumb(CxFile* hFile, int * ippLen);
	bool  Decode(FILE *hFile) { CxIOFile file(hFile); return Decode(&file); }

#if CXIMAGE_SUPPORT_ENCODE
	bool  Encode(CxFile * hFile);
	unsigned char* IppEncode(int * ippLen);
	bool  IppEncode(unsigned char* pImgInput, int imgWidth, int imgHeight,int imgChannel, int * ippLen,unsigned char* pImgOutput,int quality = 70);
