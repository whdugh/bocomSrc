...
////////////////////////////////////////////////////////////////////////////////
RGBQUAD CxImage::LABtoRGB(lab_color labColor)
{
...
////////////////////////////////////////////////////////////////////////////////
CxImage::lab_color CxImage::RGBtoLAB(RGBQUAD lRGBColor)
{
   double R,G,B;
   double CIE_L,CIE_a,CIE_b;
   double X,Y,Z;
   ...
   double ref_X = 95.047;   //Observer= 2°, Illuminant= D65
   double ref_Y = 100.000;  // Converting to RGB,it is 255,255,255 
   double ref_Z = 108.883;  // Converting to Lab ,it is 100.0,0.0,0.0
   ...
   lab_color Lab = {(float)CIE_L,(float)CIE_a,(float)CIE_b};
...
////////////////////////////////////////////////////////////////////////////////
/**
 * Adds the brightness of the image. 
 * \return true if everything is ok
 */
bool CxImage::IppLighten()
{
	if (!pDib) return false;
	...
	if(channels == 3)
		ippiAddC_8u_C3RSfs(info.pImage, lineStep, value, info.pImage, lineStep, roi, 1 );
	else if(channels == 1)
		ippiAddC_8u_C3RSfs(info.pImage, lineStep, value, info.pImage, lineStep, roi, 1 );
...
bool CxImage::Light(long brightness, long contrast)
{
	...
	BYTE cTable[256],cTable1[256]; //<nipper>
	for (int i=0;i<256;i++)	{
 ...
 */
// 
bool CxImage::IppAlphaComp(CxImage* iSrc, CxImage* iDst)
{
	//按比例缩放
	
	if (!iSrc || !pDib) return false;
	...
	roi.width = head.biWidth;
	roi.height = head.biHeight;
	int srcWidth = iSrc->GetWidth();

	channels   = head.biBitCount >> 3;
	srcchannels   = iSrc->GetBpp() >> 3;
	
	int lineStep   = roi.width*channels + DIB_PAD_BYTES(roi.width,channels);
	int dstImgStep = srcWidth*srcchannels + DIB_PAD_BYTES(srcWidth,srcchannels);

	//CxImage tmpImgFrm;
	//tmpImgFrm.Copy(*this);
	
	if(channels == 3)
		ippiAlphaCompC_8u_C3R(iSrc->GetBits(), lineStep,0,info.pImage, dstImgStep,1,info.pImage, dstImgStep,roi,ippAlphaOver );
	else if(channels == 1)
		ippiAlphaCompC_8u_C1R(iSrc->GetBits(), lineStep,1,info.pImage, dstImgStep,1,info.pImage, dstImgStep,roi,ippAlphaOut );
		...
		for(int G=0;G<256;G++)
		{
			for(int B=1;B<256;B++)
			{
				for(long lY=0;lY<height;lY++)
				{
					...
					{
						rgb = GetPixelColor(lX,lY);
						if(rgb.rgbRed == R && rgb.rgbGreen == G && rgb.rgbBlue == B)
						{
							transRGB = false;
							break;
						}
 ...
 */
// 
void CxImage::Mix(CxImage & imgsrc2, ImageOpType op, long lXOffset, long lYOffset, bool bMixAlpha ,int bTransparent ,RGBQUAD *transRGB)
{
    ...
    RGBQUAD rgb1, rgb2, rgbDest, rgbTmp;

	if (transRGB!=0) 
		rgbTmp=*transRGB; 
	else {
		rgbTmp.rgbRed=255; rgbTmp.rgbGreen=255; rgbTmp.rgbBlue=255; rgbTmp.rgbReserved=0;
	}
    ...
    for(long lY=0;lY<lHeight;lY++)
    {
        ...
        for(long lX=0;lX<lWide;lX++)
        {
...
#endif //CXIMAGE_SUPPORT_SELECTION
			{
				...
				{
					case OpTransparent:
						if(rgb2.rgbBlue == rgbTmp.rgbBlue && rgb2.rgbGreen == rgbTmp.rgbGreen && rgb2.rgbRed == rgbTmp.rgbRed)
						{
							rgbDest = rgb1; 
						}
						else
						{
							rgbDest.rgbBlue = (BYTE)(rgb1.rgbBlue*(100 - bTransparent)/100+rgb2.rgbBlue*bTransparent/100); 
							rgbDest.rgbGreen = (BYTE)(rgb1.rgbGreen*(100 - bTransparent)/100+rgb2.rgbGreen*bTransparent/100); 
							rgbDest.rgbRed = (BYTE)(rgb1.rgbRed*(100 - bTransparent)/100+rgb2.rgbRed*bTransparent/100);
							if (bEditAlpha) rgbDest.rgbReserved = (BYTE)(BYTE)(rgb1.rgbReserved*(100 - bTransparent)/100+rgb2.rgbReserved*bTransparent/100);
						}
...
bool CxImage::AutoWhiteBalance()
{
	...
	Ipp8u* tmpData = (Ipp8u*)malloc(lineStep*roi.height);
	if(channels == 3)
	{
		//RGBtoBGR(info.pImage,head.biSize);
		ippiBGRToLab_8u_C3R(info.pImage, lineStep,tmpData,lineStep, roi);
	}
	else
		return false;
	
	for(y = 0; y < head.biHeight-1;y ++)
	{	
		ptr = tmpData + lineStep*y;

		for(x = 0; x < head.biWidth-1;x ++)
		{
			a_avg += (double)(ptr[1]-128);//a
			b_avg += (double)(ptr[2]-128);//b	

			ptr += 3;
		}
	}	
		
	a_avg /= (head.biHeight*head.biWidth);
	b_avg /= (head.biHeight*head.biWidth);
	...
	for( y = 0; y < head.biHeight; y ++)
	{
		...
		{
			a_delta = (0.0-a_avg) * (ptr[0] / 255.0);//*1.05;  //Dose it have some problems?
			b_delta = (0.0-b_avg) * (ptr[0] / 255.0);//*1.1;
			
			ptr[1] += (double)a_delta;
			ptr[2] += (double)b_delta;

			ptr += 3;
		}
	}

	ippiLabToBGR_8u_C3R(tmpData, lineStep,info.pImage,lineStep, roi);

	if(tmpData!=0)free(tmpData);tmpData=0;
	...
	for(y = 0; y < head.biHeight - 1;y ++)
		...
		for( y = 0; y < head.biHeight; y ++)
			for( x = 0; x < head.biWidth; x ++)
			{
				...
				a_delta = (0.0-a_avg) * (labColor.l / 100.0)*1.05;  //Dose it have some problems?
				b_delta = (0.0-b_avg) * (labColor.l / 100.0)*1.1;
				
				labColor.a += (float)a_delta;
				labColor.b += (float)b_delta;
