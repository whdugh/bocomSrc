...
public:
typedef struct skp_ExifInfo {
    char  Version      [5];//EXIF版本
    char  CameraMake   [32];//照相机 型号
    char  CameraModel  [40];//照相机 设置
    char  DateTime     [20];//照相机 日期/时间
    int   Height, Width;//图像 Y维度像素，X维度像素
    int   Orientation;
    int   IsColor;
    int   Process;
    int   FlashUsed;//图像 闪光灯使用  0：没有使用  1：使用了
    float FocalLength;//图像 焦距
    float ExposureTime;//图像 曝光时间
    float ApertureFNumber;//图像 光圈值
    float Distance;
    float CCDWidth;
    float ExposureBias;//图像 曝光补偿值
    int   Whitebalance;//图像 白平衡   0:自动白平衡  1：手动白平衡
    int   MeteringMode;//图像 测光模式
    int   ExposureProgram;//图像 曝光程序
    int   ISOequivalent;//图像 ISO 速率
    int   CompressionLevel;//图像 压缩位/像素
	float FocalplaneXRes;
	float FocalplaneYRes;
	float FocalplaneUnits;
	float Xresolution;//照相机 X分辨率
	float Yresolution;//照相机 Y分辨率
	float ResolutionUnit;//照相机 分辨率单位  1.00000:英寸
	float Brightness;//图像 饱和度
    char  Comments[MAX_COMMENT];//文件 注释
...
struct lab_color { float l,a,b;};//高力鑫添加
...
/** \addtogroup Constructors */ //@{
	...
	CxImage(const char * filename, DWORD imagetype, bool thumbnail = false);	// For UNICODE support: char -> TCHAR
    ...
    SKPEXIFINFO  skp_exifinfo; //增加EXIFINFO信息
//@}
...
/** \addtogroup Interpolation */ //@{
	...
	//IPP interpolation methods:
	enum IPPInterMethod {
		    IPPI_INTER_NN      =1,         //  nearest neighbor interpolation
			IPPI_INTER_LINEAR  =2,         //  linear interpolation
			IPPI_INTER_CUBIC   =4,         //  cubic interpolation
			IPPI_INTER_SUPER   =8,         //  supersampling interpolation, can not be applied for image enlarging
			IPPI_INTER_LANCZOS =16,        //  interpolation with Lanczos window
			IPPI_SMOOTH_EDGE  = (1 << 31)  //use edge smoothing option in addition to one of the above methods.
	};
...
/** \addtogroup Decode */ //@{
	...
	//bool Load(const char * filename, DWORD imagetype=0);
	char* ReadIppImage(const char * filename,DWORD imagetype, int iMax, int& imageLength, int& sz);
	char* ReadIppThumb(int * ippLen);
	char* ReadThumbBmp(int * ippLen, int& sz);
	//unsigned char* ReadIppJPEGExif(int * ippLen);
	bool ReadExifInfo(const char * filename);
...
protected:
	unsigned char* ReadIppJPEGExif(int * ippLen, bool drwImage);
//@}
...
/** \addtogroup Encode */ //@{
	...
	bool IppEncode(unsigned char* imgData, int imgWidth, int imgHeight,int imgChannel, int* ippLen,unsigned char* imgOutPut,int quality = 70);
...
/** \addtogroup Transformations */ //@{
	// image operations
	bool IppRotate(float angle, IPPInterMethod const ippMethod=IPPI_INTER_LINEAR, CxImage* iDst = NULL);
	bool IppRotateLeft(IPPInterMethod const ippMethod=IPPI_INTER_LINEAR, CxImage* iDst = NULL);
	bool IppRotate180(CxImage* iDst = NULL, bool vertical = true);
	bool IppRotateRight(IPPInterMethod const ippMethod=IPPI_INTER_LINEAR, CxImage* iDst = NULL);
	...
	bool IppResize(long newx, long newy, IPPInterMethod const ippMethod=IPPI_INTER_LINEAR, CxImage* iDst = NULL);
	...
	bool IppShear(long left, long top, long right, long bottom, IPPInterMethod const ippMethod=IPPI_INTER_LINEAR, CxImage* iDst = NULL);
	bool IppShear(const RECT& rect, IPPInterMethod const ippMethod=IPPI_INTER_LINEAR, CxImage* iDst = NULL);
	...
	bool RedEyeRemove();
	bool AutoWhiteBalance();//高立鑫添加
	bool QIShrink(long newx, long newy, CxImage* const iDst = NULL);

	bool WaterMarking(CxImage* imgWatMarking);
	bool WaterMarking(CxImage* imgWatMarking, long lXOffset, long lYOffset, int bTransparent, RGBQUAD transRGB);
	bool FrameIn(CxImage* imgFrame);
	bool FrameOut(long lTopX, long lTopY, long rBottomX, long rBottomY, CxImage* imgFrame);
//@}
...
/** \addtogroup DSP */ //@{
	...
	enum ImageOpType { OpTransparent, OpAdd, OpAnd, OpXor, OpOr, OpMask, OpSrcCopy, OpDstCopy, OpSub, OpSrcBlend, OpScreen };
	RGBQUAD GetTransRGB();//tony add
	void Mix(CxImage & imgsrc2, ImageOpType op, long lXOffset = 0, long lYOffset = 0, bool bMixAlpha = false, int bTransparent = 100, RGBQUAD *transRGB=0);
	bool IppAlphaComp(CxImage* iSrc, CxImage* iDst = NULL);
...
/** \addtogroup ColorSpace */ //@{
	...
	static RGBQUAD LABtoRGB(lab_color labColor);//高立鑫添加
	static lab_color RGBtoLAB(RGBQUAD lRGBColor);//高立鑫添加
...
/** \addtogroup Protected */ //@{
	...
	void Startup(DWORD imagetype, const char* filename, bool thumbnail);
protected:
...
public://qiansen
	...
	bool                bThumbnail;   //增加判断是读取小图还是大图
	const char*         ippFileName;  //保存读取路径
	IPPSRCINFO          m_ippScrInfo; //保存原图长宽信息
	DWORD               bImgType;     //保存图象类型
	unsigned char*               ippJPEG;      //暂存ipp jpeg 格式160数据信息（目的为了用完清空）
	unsigned char*               ippJpegData;      //暂存ipp jpeg 格式240数据信息（目的为了用完清空）
//@}
};
