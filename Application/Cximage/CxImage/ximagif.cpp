...
bool CxImageGIF::Decode(CxFile *fp)
{
	...
	if(TabCol.sogct == 64 && dscgif.pflds == -27)return false;//增加排除64色的图片读取
	...
	for (BOOL bContinue = TRUE; bContinue; )
	{
		...
		if (bPreviousWasNull || ch==0)
		{
			...
			case ',': // image
				...
				//add these 2 lines:更改添加
				if (dscgif->scrwidth==0)  dscgif->scrwidth = image.w;
				if (dscgif->scrheight==0) dscgif->scrheight = image.h;
